...
def get_by_code(code: str) -> Optional[PortDTO]:
    db.execute("""SELECT p.code,p."name",p.parent_slug FROM ports p WHERE p.code = %(code)s """, {"code": code})
    row = db.fetchone()
    return PortDTO(code=row[0], name=row[1], parent_slug=row[2]) if row else None


def get_all_by_region_slug(region_slug: str) -> List[PortDTO]:
    """
    ...
    """
    # The ordercol is good to have to track the recursion for debugging + testing validity
    db.execute("""WITH RECURSIVE all_ports_in_region AS (
