...
class PriceRepository:
    ...
    def get_avg_price(self, price_request_dao: PriceRequestDAO) -> List[AvgPriceDTO]:
        if not price_request_dao.origins or not price_request_dao.destinations:
            return []
        self.db.execute("""select pr."day" as day, Avg(pr.price) as avg_price, count(pr.price) as count 
                        from prices pr 
                        ...
                        order by pr."day" """,
                        {"orig": tuple([origin for origin in price_request_dao.origins]),
                         "dest": tuple([destination for destination in price_request_dao.destinations]),
        ...
        return [
            AvgPriceDTO(average_price=round(result[1], 2), day=dt.datetime.combine(result[0], dt.datetime.min.time()),
